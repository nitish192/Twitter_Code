<?php
 ...
 class Follow extends User{
	...
	public function follow($followID, $user_id, $profileID){
		...
		$this->create('follow', array('sender' => $user_id, 'reciever' => $followID, 'followOn' => $date));
		$this->addFollowCount($followID, $user_id);
		$stmt = $this->pdo->prepare('SELECT `user_id`, `following`, `followers` FROM `users` LEFT JOIN `follow` ON `sender` = :user_id AND CASE WHEN `reciever` = :user_id THEN `sender` = `user_id` END WHERE `user_id` = :profileID');
	...
	public function unfollow($followID, $user_id, $profileID){
		$this->delete('follow', array('sender' => $user_id, 'reciever' => $followID));
	...
	public function followingList($profileID, $user_id, $followID){
		$stmt = $this->pdo->prepare("SELECT * FROM `users` LEFT JOIN `follow` ON `reciever` = `user_id` AND CASE WHEN `sender` = :profileID THEN `reciever` = `user_id` END WHERE `sender` IS NOT NULL ");
	...
	public function followersList($profileID, $user_id, $followID){
		$stmt = $this->pdo->prepare("SELECT * FROM `users` LEFT JOIN `follow` ON `sender` = `user_id` AND CASE WHEN `reciever` = :profileID THEN `sender` = `user_id` END WHERE `user_id` and `reciever` IS NOT NULL");
		$stmt->bindParam(":profileID", $profileID, PDO::PARAM_INT);
		$stmt->execute();
		$followings = $stmt->fetchAll(PDO::FETCH_OBJ);
		foreach ($followings as $following) {
	...
	public function whoToFollow($user_id, $profileID){
		$stmt = $this->pdo->prepare("SELECT * FROM `users` WHERE `user_id` != :user_id AND `user_id` NOT IN (SELECT `reciever` FROM `follow` WHERE `sender` = :user_id) ORDER BY rand() LIMIT 5");
