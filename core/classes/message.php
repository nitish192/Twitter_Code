     ...
     class Message extends User{
        ...
        public function recentMessages($user_id){
          ...
          public function deleteMsg($messageID, $user_id){
            $stmt = $this->pdo->prepare("DELETE FROM `messages` WHERE `messageID` =:messageID AND `messageForm` =:user_id OR `messageID` =:messageID AND `messageTo` =:user_id");
          ...
          public function notification($user_id){
            $stmt = $this->pdo->prepare("SELECT * FROM `notification` N LEFT JOIN `users` U ON N.`notificationForm` = U.`user_id` LEFT JOIN `tweets` T ON N.`target` = T.`tweetID` LEFT JOIN `likes` L ON N.`target` = L.`likeOn` LEFT JOIN `follow` F ON N.`notificationForm` = F.`sender` AND N.`notificationFor` = F.`reciever` WHERE N.`notificationFor` = :user_id AND N.`notificationForm` != :user_id GROUP BY `ID` ");
          ...
          public function sendNotification($get_id, $user_id, $target, $type){
           ...
           $this->create('notification', array('notificationFor' => $get_id, 'notificationForm' => $user_id, 'target' => $target, 'type' => $type, 'time' => $date));
          }
